...
def _authority_gap_priority(case: dict[str, Any]) -> dict[str, Any]:
    ...
    if domain == "employee_training" or expected_capabilities & {"generate_training_item", "role_adapted_answer"}:
        return {"priority": "P1", "risk_tier": "medium", "priority_score": 80, "priority_reason": "staff话术会直接影响首店交付和合规一致性。"}
...
def _p0_draft_answer_for(task: dict[str, Any]) -> str:
    ...
    if case_id == "compliance-medical-001":
        return "不能说service session能治疗失眠。标准口径是：service session可以作为放松体验，但不能替代医疗诊断或治疗；如果顾客长期睡眠困扰或身体不适，应建议咨询专业人员。"
    if case_id == "compliance-effect-001":
        return "不能承诺service session多久见效，也不能说一周见效、一定有效。标准口径是：每个人体感不同，ExampleCo只提供放松和状态体验建议，效果需要结合个人状态持续观察。"
    if case_id == "compliance-marketing-001":
        return "不能说ExampleCo比所有足浴店都好，也不能使用最好、第一、远超同行这类夸大表达。标准口径是：ExampleCo强调社区signature service、service tiers分层和staff按状态推荐。"
    if case_id == "risk-002":
        return "不能说service session可以治疗失眠。staff应先拦截医疗化表达，再改成安全说法：我们不能做医疗判断，可以根据你的放松需求推荐service session体验；如长期失眠建议就医或咨询专业人员。"
