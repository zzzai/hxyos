...
HXY_TERMS = ["ExampleCo", "service tiers", "entry tier", "adaptive tier", "recovery tier", "continuity tier", "community site", "visible preparation"]
COMPETITOR_ONLY_TERMS = ["CompetitorA", "CompetitorB", "CompetitorD", "LANN", "蘭泰式", "CompetitorE", "CompetitorF", "素可泰", "CompetitorC", "贡小推"]
REVIEW_DOMAIN_HINTS = {
    "store_model": ["site operating model", "小店", "community site", "面积", "房间", "人员配置", "单店模型", "坪效", "人效"],
    "competitor_research": ["竞品", "CompetitorA", "CompetitorB", "CompetitorD", "LANN", "CompetitorE", "CompetitorF", "品牌数据", "排名"],
    "unit_economics": ["投资", "回本", "客单价", "利润", "成本", "租金", "毛利", "现金流"],
    "product_system": ["entry tier", "adaptive tier", "recovery tier", "continuity tier", "草本", "service session", "产品体系"],
    "employee_script": ["staff", "话术", "frontline staff", "接待", "推荐", "培训", "复训"],
    "risk_boundary": ["治疗", "治愈", "保证", "一定有效", "稳赚", "医学", "禁用", "不能承诺"],
    "brand_positioning": ["定位", "品牌", "核爆点", "service clinic", "轻养生"],
}
...
def _domain_for(text: str) -> str:
    if any(term in text for term in ["定位", "品牌", "核爆点", "service clinic"]):
        return "brand_positioning"
    if any(term in text for term in ["entry tier", "adaptive tier", "recovery tier", "continuity tier", "产品", "service session"]):
        return "product_system"
    if any(term in text for term in ["staff", "话术", "训练", "frontline staff"]):
        return "employee_training"
    if any(term in text for term in ["partnership", "回本", "partnership", "收益"]):
...
def _risk_flags(text: str) -> list[str]:
    if any(term in text for term in OVERCLAIM_TERMS):
...
def _question_for_review_item(item: dict[str, Any]) -> str:
    ...
    templates = {
        "brand_positioning": "ExampleCo的品牌定位应该怎么说？",
        "product_system": "service tiers或产品体系应该怎么解释？",
        "store_model": "ExampleCosite operating model的关键判断是什么？",
        "competitor_research": "这条竞品信息对ExampleCo有什么参考价值？",
        "unit_economics": "这条单店模型或投资测算信息能否作为内部判断依据？",
        "employee_script": "staff应该如何按标准口径表达？",
        "risk_boundary": "这条表达有哪些合规风险，应该如何改写？",
    }
    return templates.get(group, "这条候选知识是否可以进入ExampleCo标准口径？")
...
def build_answer_card_drafts(review_items: list[dict[str, Any]], limit: int = 10) -> dict[str, Any]:
    ...
    for item in review_items:
        ...
        if risk_flags:
            ...
            answer = f"当前为草稿，不能直接发布。候选资料显示：{claim} 复核通过后可整理为ExampleCo标准口径，并补充适用场景和引用来源。"
        drafts.append(
            {
